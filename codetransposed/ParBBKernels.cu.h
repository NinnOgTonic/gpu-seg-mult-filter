...
class MyInt4 {
  public:
    ...
    __host__ inline void selAdd(int i, int val) {
        if     (i==0) { x += val; }
        else if(i==1) { y += val; }
        else if(i==2) { z += val; }
        else          { w += val; }
...
class Mod4 {
  public:
    ...
    static __device__ inline void incWithAccumDiffSgm(
        MyInt4& acc,
        MyInt4& c_cnts,
        bool include_segment,
        MyInt4& s_cnts
    ) {
        if(include_segment) {
            acc.w += s_cnts.x + s_cnts.y + s_cnts.z;
            acc.z += s_cnts.x + s_cnts.y;
            acc.y += s_cnts.x;
        }

        acc.w += c_cnts.x + c_cnts.y + c_cnts.z;
        acc.z += c_cnts.x + c_cnts.y;
        acc.y += c_cnts.x;
    }
};
...
sgmMapVctKernel(typename MapLambda::InType*     d_in,
                ...
                typename MapLambda::ExpType*    segment_counters,
                const unsigned int*             flags, // Flags on the form [1,0,0,0,1,0,0] can be derrived by a scan of segment flags
                const unsigned int*             segment_ids, // Flags on the form [1,1,2,2,2,3,3,4,4,4,...] can be derrived by a scan of segment flags
                unsigned int*                   segment_idxs, // Uninitialised, is set here
    ...
    volatile int* segment_acc = ((volatile int*)map_sh_mem) + MapLambda::cardinal*(blockDim.x + segment_ids[gid] - 1);

    printf("shared mem: %p, chunk_acc: %p, segment_acc: %p, gid: %d, segment_ids[gid]: %d\n", map_sh_mem, chunk_acc, segment_acc, gid, segment_ids[gid]);

    bool write_to_segment = flags[gid] == 1;
    int segments_touched = (write_to_segment ? 1 : 0);
    int first_segment_touched = -1;

    // Calculate chunk flags for use in later kernels (2. segmented scan, ...)
    int chunk_flag;
    if(flags[gid]) {
        chunk_flag = 1;
    } else if(gid > 0 && segment_ids[gid] != segment_ids[gid - 1]) {
        chunk_flag = 1;
    } else {
        chunk_flag = 0;
    }
    ...
    if(gid < D_HEIGHT) {
        ...
        for(i = 0; i < D_WIDTH; i++, gid += D_HEIGHT) {
            if(flags[gid] == 1) {
                segment_idxs[segment_ids[gid]] = gid;
                printf("GID %d SAW FLAG: %d, SEG_ID: %d\n", gid, flags[gid], segment_ids[gid]);
                segment_acc = ((volatile int*)map_sh_mem) + MapLambda::cardinal*(blockDim.x + segment_ids[gid] - 1);
		for(n = 0; n < MapLambda::cardinal; n++) {
		    //segment_acc[n] = MapLambda::identity();
		    segment_acc[n] = MapLambda::identity();
		}
                if(first_segment_touched < 0) {
                    first_segment_touched = segment_ids[gid] - 1;
                }
    		write_to_segment = 1;
                segments_touched += 1;
	    }
        ...
        */
        ...
        printf("####. gid: %d, segment_ids[gid]: %d\n", gid, segment_ids[gid]);
        if(segments_touched > 0) {
    	    for(n = first_segment_touched; n < first_segment_touched + segments_touched; n++) {
                printf("WRITING TO SEGMENT COUNTER %d\n", n);
	        segment_acc = ((volatile int*)map_sh_mem) + MapLambda::cardinal*(blockDim.x + n);
	        for(i = 0; i < MapLambda::cardinal; i++) {
		    ((int*)(segment_counters + n))[i] = segment_acc[i];
	        }
	    }
	}
        ...
        for(i = 0; i < MapLambda::cardinal; i++) {
            ((int*)(chunk_counters + gid))[i] = chunk_acc[i];
	}
...
template<class OP>
__global__ void
sgmWriteMultiKernelTest(   typename OP:: InType* d_in,  int* cond_res,
                    typename OP::ExpType* perm_chunk,
                    typename OP:: InType* d_out,
		    const unsigned int*   segment_ids,
                    const unsigned int d_height,
...
template<class OP>
__global__ void
sgmWriteMultiKernel(typename OP:: InType* d_in,
                    const unsigned int*   chunk_flags,
                    typename OP::ExpType* chunk_counters,
                    typename OP::ExpType* chunk_counters_scanned,
                    typename OP::ExpType* segment_counters,
		    const unsigned int*   segment_ids,
		    const unsigned int*   segment_idxs,
                    typename OP::InType* d_out,
                    const unsigned int    D_HEIGHT,
                    const unsigned int    D_WIDTH
) {
    ...
    extern __shared__ char map_sh_mem[];
    volatile int *sh_mem = (volatile int*)map_sh_mem;
    //M chunk_mem[D_WIDTH];

    M acc0;
    printf("ACC0 PRE-INITIAL IS: X: %d, Y: %d, Z: %d, W: %d\n", acc0.x, acc0.y, acc0.z, acc0.w);
    int segment_id = segment_ids[gid];
    //int num_segments = 0;
    int i;

    // 1. Read input data into local chunk memory
    //for(i = 0, tmp_id = gid; i < D_WIDTH; i++, tmp_id += D_HEIGHT) {
        //chunk_mem[i] = d_in[tmp_id];
    //}

    // 2. Figure out where this chunk should write its data to
    acc0 = M();

    // First find correct starting indexes within the chunk for all classes
    OP::incWithAccumDiffSgm(acc0, chunk_counters[gid], gid == segment_idxs[segment_id], segment_counters[segment_id]);
    printf("GID: %d, SEGMENT ID: %d, ACC0 INITIAL IS: %d, %d, %d, %d\n", gid, segment_id, acc0.x, acc0.y, acc0.z, acc0.w);

    // If we aren't the chunk writing to segment counter, offset by segment counter counts.
    if(gid != segment_idxs[segment_id]) {
        acc0 += segment_counters[segment_id];
        printf("SEGMENT COUNTERS FOR SEGID %d ARE: X: %d; SEG START IDX IS: %d\n", segment_id, segment_counters[segment_id].x, segment_idxs[segment_id]);
    }

    // Add offset from aggregated chunk counters in same segment *before* our chunk.
    if(chunk_flags[gid] != 1) {
        acc0 += chunk_counters_scanned[gid - 1];
    }

    // With offset within segment set, add segment start index for global idx
    int idx = segment_idxs[segment_id];
    //M tmp = M(idx, idx, idx, idx);
    M tmp = M(40 * (segment_id - 1), 40 * (segment_id - 1), 40 * (segment_id - 1), 40 * (segment_id - 1));
    acc0 += tmp;

    printf("GID IS: %d, ACC0 IS: X: %d, Y: %d, Z: %d, W: %d\n", gid, acc0.x, acc0.y, acc0.z, acc0.w);
    return;
    
    // 3. Write data from chunk memory to proper places in shared memory
    //for(i = 0; i < D_WIDTH; i++) {
    if(gid < D_HEIGHT) {
        for(i = 0, tmp_id = gid; i < D_WIDTH; i++, tmp_id += D_HEIGHT) {
            //int idx = acc0.selInc(OP::apply(chunk_mem[i]));
            int elem = d_in[tmp_id];
            int idx = acc0.selInc(OP::apply(elem));
            //sh_mem[idx - 1] = chunk_mem[i];
            sh_mem[idx - 1] = OP::apply(elem);
            sh_mem[idx - 1] = 66;
            printf("sh_mem[%d] JUST SET. IT IS %d\n", idx, sh_mem[idx - 1]);
             
            //printf("KERNEL SAYS APPLY ELEM IS %d WITH APPLY RESULT: %d\n", elem, OP::apply(elem));
            //sh_mem[idx - 1] = elem;
        }
    }
    ...
    if(gid < D_HEIGHT) {
        // 4. Write ordered shared memory back to global memory
        for(i = 0, tmp_id = gid; i < D_WIDTH; i++, tmp_id += D_HEIGHT) {
            d_out[tmp_id] = sh_mem[tmp_id];
            printf("sh_mem[%d] IS %d\n", tmp_id, sh_mem[tmp_id]);
            //d_out[tmp_id] = 44;
        }
    }
...
/*
    ...
    for(i = 0; i < D_HEIGHT; i++) {
        //d_out[tmp_id + i] = 0;
        d_out[tmp_id + i] = sh_mem[tmp_id + i];
    }
*/
}
...
template<class OP>
__global__ void
sgmWriteMultiKernel2(typename OP:: InType* d_in,  int* cond_results,
                    typename OP::ExpType* segment_counters_scanned,
                    typename OP:: InType* d_out,
		    const unsigned int*   segment_ids,
                    const unsigned int    d_height,
                    const unsigned int    orig_size,
...
//    int col = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    // 1. vertically scan sequentially CHUNK elements, result in acc0
    unsigned int tmp_id = blockIdx.x*blockDim.x + threadIdx.y*CHUNK*d_height + threadIdx.x;
    int segment_id = segment_ids[tmp_id];
    //int num_segments = 0;

//    if(col < d_height)
    // TODO FIXME... is it really CHUNK we should be looping to? Probably yes.
    for(k = 0; k < CHUNK; k++, tmp_id+=d_height) {
        acc0.selInc(cond_results[tmp_id]);
        /*
        if(segment_id == segment_ids[tmp_id]) {
            acc0.selInc(cond_results[tmp_id]);
        } else {
            ind_sh_mem[threadIdx.x*(blockDim.y+1)+threadIdx.y + num_segments] = acc0;
            // TODO FIXME: We can at most touch 2 segments or intermediary segments are deleted. Problem?
            acc0.zeroOut();
            segment_id = segment_ids[tmp_id];
            num_segments++;
        }
        */
    }
    ...
    // TODO FIXME, should scan scan globally, or within segments? Assume globally, easiest.
    // 2. vertical warp-scan of the results from the seq scan step, put result back in acc0
    scanIncWarp<typename OP::AddExpType,M>(ind_sh_mem + threadIdx.y*(blockDim.x+1), threadIdx.x);
    __syncthreads();
    {
...
//    if(col < d_height)
    { // 3. adjust acc0 to reflect block-level indices of the multi-index.
        tmp_id = blockIdx.x*blockDim.x;
        OP::incWithAccumDiff(acc0, arrtmp, segment_counters_scanned + tmp_id - 1,
                             segment_counters_scanned + tmp_id + threadIdx.x - 1,
                             segment_counters_scanned + min(tmp_id + blockDim.x, d_height) - 1);
...
//    if (col < d_height)
    ...
    // 5. Finally, the shared memory is traverse in order and
    //    and the filtered array is written to global memory;
    //    Since all the elements of an equiv-class are contiguous
    //    in shared memory (and global memory), the uncoalesced
    //    writes are minimized.
    {
        printf("STARTING LAST STAGE OF WRITE BACK\n");
        int* blk_vlst= ((int*)(segment_counters_scanned + d_height - 1)); // very last (row) scan result
        k   = threadIdx.y*blockDim.x + threadIdx.x;
        unsigned int total_len = blockDim.x*blockDim.y*CHUNK;
        for( ; k < total_len; k+=blockDim.x*blockDim.y) {
            unsigned int glb_ind = 0, loc_ind = k;
            ...
            tmp_id = glb_ind + loc_ind + (blockIdx.x > 0) *
                     ((int*) (segment_counters_scanned + blockIdx.x*blockDim.x - 1))[tmp_id]; // blk_beg;
            if(tmp_id < orig_size) {
                d_out[tmp_id] = elm_sh_mem[myHash(k)];
