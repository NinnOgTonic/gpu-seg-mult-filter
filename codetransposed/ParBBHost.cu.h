...
sgmMultiFilter( const unsigned int      num_elems,
    ...
    cudaMalloc((void**)&segment_idxs, num_segments * sizeof(int));
    ...
    { // 2. The Map Condition Kernel Call
        ...
        sgmMapVctKernel<DISCR><<<num_blocks, block_size, SH_MEM_MAP>>>
            (d_tr_in, cond_results, chunk_counters, segment_counters, flags_tr, segment_ids_tr, segment_idxs, chunk_flags, D_HEIGHT, D_WIDTH);
    }
    cudaThreadSynchronize();
    ...
    printf("SEGMENT_IDXS IS:\n");
    cuprintf((int*)segment_idxs, 50);
    ...
    { // 3. the inclusive scan of the condition results
        ...
        // TODO: mapAggregateCounters commented out. We don't want to use it anymore, right?
        ...
        // Get globally incrementing counts for each segment
        //const unsigned int block_size = 128; // Already set
        //scanInc<typename DISCR::AddExpType,typename DISCR::ExpType>
        //    (block_size, num_segments, segment_counters, segment_counters + num_segments);
        //cudaThreadSynchronize();
    ...
    { // 4. the write to global memory part
        const unsigned int block_size = 64; //256;
        const unsigned int num_blocks = (D_HEIGHT + block_size - 1) / block_size;
        const unsigned int SH_MEM_MAP  = num_elems * sizeof(int);

        // map the condition
        sgmWriteMultiKernel<DISCR><<<num_blocks, block_size, SH_MEM_MAP>>>
            (d_tr_in, chunk_flags, chunk_counters, chunk_counters + D_HEIGHT, segment_counters, segment_ids_tr, segment_idxs, d_out, D_HEIGHT, D_WIDTH);
    }
    cudaThreadSynchronize();
    printf("WRITE BACK DONE\n");
    printf("D_OUT IS:\n");
    cuprintf(d_out, 2048);
    gettimeofday(&t_end, NULL);
